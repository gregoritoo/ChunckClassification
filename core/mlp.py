...
class MLPModel(nn.Module):
    def __init__(self, input_sizes,dropout_ratio=0.3):
        ...
        use_dropout[-1] = False
        self.layers = nn.ModuleList([LinearLayer(input_sizes[i],input_sizes[i+1],dropout_ratio=use_dropout[i],
                                use_dropout=use_dropout) for i in range(len(input_sizes)-1) ])
        self.out = nn.Sigmoid()
