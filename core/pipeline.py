...
class Pipeline:
    def __init__(
        self, trained_scaler, trained_prediction_model, trained_bert_model, device="cpu"
    ):
    ...
    def _get_feedbacks(self):
        true_sentence = [
            sentence
            for sentence in self.data["CustomerFeedback"].values
            if isinstance(sentence, str)
        ]
        true_sentence_index = [
            i
            for i, sentence in enumerate(self.data["CustomerFeedback"].values)
            if isinstance(sentence, str)
        ]
    ...
    def _process_feedbacks(self):
        ...
        for position, index in enumerate(true_sentence_index):
            self.data.at[index, "SentimentsDictionnaryBert"] = predictions_sentiments[
                position
            ]
    ...
    def _features_engineering(self):
        self.data = convert_df_to_numeric_df(
            self.data, ["RowNumber", "CustomerId", "Surname", "CustomerFeedback"]
        )
        self.data["RatioSalary"] = self.data["EstimatedSalary"] / (
            self.data["Balance (EUR)"] + 1
        )
        self.data["RatioProducst"] = self.data["NumberOfProducts"] / (
            self.data["Tenure"] + 1
        )
        self.data["RatioCards"] = (
            self.data["HasCreditCard"] & self.data["IsActiveMember"]
        )
        self.data["ProductSalary"] = (
            self.data["Balance (EUR)"] * self.data["CreditScore"]
        )
        self.scale_data()
    ...
    def interpret_bert(self):
        explainer = SequenceClassificationExplainer(
            self.bert_model.model, self.bert_model.tokenizer
        )
        corpus_by_feedback = [
            nltk.sent_tokenize(text) if isinstance(text, str) else 0
            for text in self.feedbacks
        ]
